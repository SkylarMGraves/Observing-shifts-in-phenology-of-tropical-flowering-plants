...
df <- df %>% filter(species %in% index$species) %>%
  mutate(JulianDay = as.numeric(JulianDay))

# Keep only one row per unique combination of species, JulianDay, and Year
df <- df %>% distinct(species, JulianDay, Year, .keep_all = TRUE)
